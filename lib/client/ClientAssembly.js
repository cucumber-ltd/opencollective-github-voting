...
module.exports = class ClientAssembly {
  constructor({ domNode, restClient }) {
    this._domNode = domNode
    this._transferCommands = new HttpTransferCommands(restClient)
    this._accountQueries = new HttpAccountQueries(restClient)
  }

  start() {
    // TODO: Add an AccountNumberProvider component that will fetch the account number of the logged-in user
    const accountNumber = { number: '@aslakhellesoy', currency: 'votes' }
    const props = {
