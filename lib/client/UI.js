  ...
  componentDidMount() {
    const accountUpdated = async accountNumber => {
      if (accountNumber.currency === this.props.accountNumber.currency) {
        try {
          const accounts = await this.props.accountQueries.getAccounts(this.props.accountNumber.currency)
    ...
    this.props.accountQueries.on('accountUpdated', accountUpdated)
  }
