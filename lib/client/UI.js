...
class VotingApp extends Component {
  constructor(props) {
    ...
    this.transferAmountTo = (toAccountNumber, amount) => {
      this.props.votingPort.transfer(this.props.accountNumber, toAccountNumber, amount)
        .then(() => {
  ...
  componentDidMount() {
    const accountUpdated = async accountNumber => {
      if (accountNumber.currency === this.props.accountNumber.currency) {
        try {
          const accounts = await this.props.accountStore.getAccounts(this.props.accountNumber.currency)
    ...
    this.props.accountStore.on('accountUpdated', accountUpdated)
  }
...
const Account = ({ account, transferAmountTo }) => {
  ...
  return hx`
    <div data-account-number="${account.accountNumber.owner}:${account.accountNumber.currency}" data-type="Account">
      ...
      <span aria-label="Uid">${account.uid}</span>
