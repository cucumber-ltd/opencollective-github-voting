...
module.exports = class CommitDaysUpdater {
  ...
  async importCommits({ owner, repo, author, token }) {
    ...
    for (const [committer, commitDays] of commitDaysByCommitter) {
      const accountHolder = await this._bankQueries.getAccountHolderByGitHubUser(committer)
      if (accountHolder) {
        ...
        if (account) {
          ...
          await this._accountCommands.creditAccount({ accountId, amount, uniqueReference })
        }
      }
    }
  }
